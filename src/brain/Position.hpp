...
/**
 * Print position in human-readable form
 * @param position the position to print
 */
static void print64(uint64_t position);
...
public:
    ...
    static const int FOUR = 4; // how many disk align to win
     ...
     * @param seq: a sequence of digits corresponding to the 0-based index of the column played.
     *
     ...
     */
    unsigned int playSeq(std::string seq) {
        for(unsigned int i = 0; i < seq.size(); i++) {
            int col = seq[i] - '0'; // 0-based
     ...
     */
    int playCol(int col) {
        uint64_t move = (mask + bottom_mask_col(col)) & column_mask(col);
        play(move);
        return log2((move >> (HEIGHT+1) * col));
    }
     ...
     * @return -1 nobody wins, 0 drawn (no more moves), 1 first player wins, 2 second player wins
     */
    int whoWin() {
        ...
        if (alignment(current_position ^ mask)) {
            return lastPlayer(); // Last player wins
        }

        // Current player has an alignment, should not happens
        assert(!alignment(current_position ));
     ...
     */
    std::array<Move, FOUR> getWinningPosition() {
        std::array<Move, FOUR> arr;

        std::bitset<64> bs = std::bitset<64>(current_position ^ mask); // last player position

        // Check orizzontal
        for(int r = 0; r < HEIGHT; r++) {
            for(int c = 0; c <= WIDTH - FOUR; c++) {
                bool ok = true;
                for(int p = 0; p < FOUR; p++) {
                    int m = r + c  * WIDTH + p  * WIDTH;
                    ok &= bs[m] == 1;
                }
                if (ok) {
                    for(int p = 0; p < FOUR; p++) {
        ...
        for(int r = 0; r <= HEIGHT - FOUR; r++) {
            for(int c = 0; c <= WIDTH - FOUR; c++) {
                bool ok = true;
                for(int p = 0; p < FOUR; p++) {
                    int m = c * WIDTH + r + p * WIDTH + p;
                    assert(m < WIDTH * (HEIGHT + 1));
                    ok &= bs[m] == 1;
                }
                if (ok) {
                    for(int p = 0; p < FOUR; p++) {
        ...
        // Check diagonal 2
        for(int r = HEIGHT - 1; r > HEIGHT - FOUR; r--) {
            for(int c = 0; c <= WIDTH - FOUR; c++) {
                bool ok = true;
                for(int p = 0; p < FOUR; p++) {
                    int m = c * WIDTH + r + p * WIDTH - p;
 ...
 */
static void print64(uint64_t position) {
    std::bitset<64> bs = std::bitset<64>(position);
    for(int c = 0; c < Position::WIDTH; c++) {
        for(int r = 0; r < Position::HEIGHT + 1; r++) {
            //std::cerr << (Position::HEIGHT - c) + r * Position::WIDTH << ' ';
            std::cerr << bs[(Position::HEIGHT - c) + r * Position::WIDTH] << ' ';
        }
        std::cerr << "\n";
    }
    std::cerr << "\n";
