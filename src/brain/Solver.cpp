...
/**
 * Reccursively score connect 4 position using negamax variant of alpha-beta
 * algorithm.
 ...
 * @param: alpha < beta, a score window within which we are evaluating the
 * position. depth, the depth it should search for (-1 infinite)
 *
 * @return the exact score, an upper or lower bound score depending of the case:
 * - if actual score of position <= alpha then actual score <= return value <=
 * alpha
 * - if actual score of position >= beta then beta <= return value <= actual
 * score
...
int Solver::negamax(const Position &P, int alpha, int beta, int depth) {
  ...
  if (P.nbMoves() >=
      Position::WIDTH * Position::HEIGHT - 2) // check for draw game
    return 0;

  int min = -(Position::WIDTH * Position::HEIGHT - 2 - P.nbMoves()) /
  ...
  int max = (Position::WIDTH * Position::HEIGHT - 1 - P.nbMoves()) /
  ...
  if (int val = transTable.get(key)) {
    if (val > Position::MAX_SCORE - Position::MIN_SCORE +
                  1) { // we have an lower bound
      min = val + 2 * Position::MIN_SCORE - Position::MAX_SCORE - 2;
      if (alpha < min) {
        alpha =
            min; // there is no need to keep beta above our max possible score.
        if (alpha >= beta)
          return alpha; // prune the exploration if the [alpha;beta] window is
                        // empty.
      }
      ...
      if (beta > max) {
        ...
        if (alpha >= beta)
          return beta; // prune the exploration if the [alpha;beta] window is
                       // empty.
      }
    }
  }
  ...
  if (int val = book.get(P))
    return val + Position::MIN_SCORE -
           1; // look for solutions stored in opening book
  ...
  while (uint64_t next = moves.getNext()) {
    ...
    int score = -negamax(
        P2, -beta, -alpha,
        depth); // explore opponent's score within [-beta;-alpha] windows:
    // no need to have good precision for score better than beta (opponent's
    // score worse than -beta) no need to check for score worse than alpha
    // (opponent's score worse better than -alpha)

    if (score >= beta) {
      transTable.put(key, score + Position::MAX_SCORE -
                              2 * Position::MIN_SCORE +
                              2); // save the lower bound of the position
      return score; // prune the exploration if we find a possible move better
                    // than what we were looking for.
    }
    if (score > alpha)
      alpha = score; // reduce the [alpha;beta] window for next exploration, as
                     // we only
    // need to search for a position that is better than the best so far.
  }

  transTable.put(key, alpha - Position::MIN_SCORE +
                          1); // save the upper bound of the position
  return alpha;
}

int Solver::solve(const Position &P, int depth, bool weak) {
  if (P.canWinNext()) // check if win in one move as the Negamax function does
  ...
  while (min < max) { // iteratively narrow the min-max exploration window
    ...
    int r = negamax(P, med, med + 1,
                    depth); // use a null depth window to know if the actual
                            // score is greater or smaller than med
    if (r <= med)
      max = r;
    else
      min = r;
...
static int getMoveColumn(uint64_t move) {
  ...
  assert(false &&
         "Unable to find a column for the move"); // I should never be here
}
...
// Constructor
Solver::Solver() {
  for (int i = 0; i < Position::WIDTH;
       i++) // initialize the column exploration order, starting with center
    // columns
    columnOrder[i] =
        Position::WIDTH / 2 +
        (1 - 2 * (i % 2)) * (i + 1) /
            2; // example for WIDTH=7: columnOrder = {3, 4, 2, 5, 1, 6, 0}
}
