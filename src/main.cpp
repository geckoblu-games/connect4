...
int main(int argc, char *argv[]) {
  ...
  qmlRegisterUncreatableType<LevelClass>("connect4", 1, 0, "Level",
                                         "Not creatable as it is an enum type");

  // Set the working dir to the application dir so that GameModel could find the
  // opening books in the same directory of the executable qDebug() <<
  ...
  engine.rootContext()->setContextProperty(
      "gamemodel",
      gamemodel); // the object will be available in QML with name "gamemodel"

  engine.load(url);

  auto myElement = engine.rootObjects().first()->findChild<QObject *>(
      QLatin1String("board"));
  // qDebug() << myElement;

  QObject::connect(gamemodel, SIGNAL(moveChoosed(QVariant)), myElement,
                   SLOT(moveChoosed(QVariant)));
